...
export const uploadImage = (
  ...
  const name = `${crypto.randomUUID({ disableEntropyCache: true })}${prefix && `-${prefix}`}${path.extname(
    file.originalname
  )}`;
 ...
 * @param hasUnlink
 */
export const cleaningDirectory = async (pathDir: string, hasUnlink?: (file: string) => boolean) => {
